...
const menuData = [
  ...
  },
  {
    ...
    badge: null,
    image: banner1,
    accent: "hsl(150,45%,28%)",
    accentLight: "hsl(150,45%,96%)",
    emoji: "🫖",
    tag: "Most Popular",
  },
  {
    ...
    price: 140,
    valueNote: "₹14/cup — save ₹1 a cup vs. Premium",
    badge: "Bulk",
    image: banner2,
...
const MenuSection = () => {
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(160deg, hsla(0,0%,99%,0.85) 0%, hsla(4,85%,98%,0.85) 50%, hsla(0,0%,99%,0.85) 100%)",
    ...
    >
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        {/* Menu Cards */}
        <motion.ul
          className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7 max-w-5xl mx-auto list-none p-0"
          variants={container}
          initial="hidden"
          whileInView="show"
          viewport={{ once: true }}
        >
          {menuData.map((m) => (
            <motion.li
              key={m.id}
              variants={item}
              className="group relative bg-white rounded-3xl overflow-hidden border border-border/40 flex flex-col"
              style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}
              whileHover={{
                y: -8,
                boxShadow: `0 20px 56px ${m.accent}22, 0 6px 16px ${m.accent}14`,
              }}
              transition={{ duration: 0.3 }}
            >
              {/* Card top tag */}
              <div
                className="absolute top-3 left-3 z-20 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide text-white shadow-md"
                style={{
                  background: m.accent,
                  boxShadow: `0 2px 8px ${m.accent}55`,
                }}
              >
                {m.tag}
              </div>
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <div className="flex items-center gap-3 mt-5 mb-1 flex-wrap">
                  <span
                    className="text-3xl font-black px-3 py-1 rounded-xl"
                    style={{
                      color: m.accent,
                      background: `${m.accent}12`,
                      border: `1.5px solid ${m.accent}22`,
                      textShadow: `0 2px 8px ${m.accent}30`,
                    }}
                  >
                    ₹{m.price}
                  </span>
                ...
                {!m.valueNote && <div className="mb-4" />}
                ...
                <motion.a
                  ...
                  aria-label={`Order ${m.name} via the Food Man app`}
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-white text-sm font-black shadow-lg transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                  style={{
        ...
        <motion.div
          className="mt-10 max-w-5xl mx-auto rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center gap-7 shadow-xl border overflow-hidden relative"
          style={{
            background: "linear-gradient(135deg, hsl(4,85%,36%) 0%, hsl(4,85%,24%) 100%)",
            borderColor: "hsl(4,85%,28%)",
            boxShadow: "0 24px 64px hsla(4,85%,40%,0.35), 0 4px 16px hsla(4,85%,40%,0.2)",
          }}
        ...
        >
          {/* Decorative circles */}
          <div className="absolute -top-10 -right-10 w-44 h-44 rounded-full bg-white/5" aria-hidden="true" />
          <div className="absolute -bottom-8 -left-8 w-32 h-32 rounded-full bg-white/5" aria-hidden="true" />

          <div className="text-5xl z-10" aria-hidden="true">📦</div>
          ...
          <motion.a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Order bulk tea for corporate events via the Food Man app"
            whileHover={{ scale: 1.05, y: -2 }}
            whileTap={{ scale: 0.97 }}
            className="flex-shrink-0 flex items-center gap-2 px-7 py-3.5 rounded-2xl font-black text-sm shadow-xl z-10 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[hsl(4,85%,24%)]"
